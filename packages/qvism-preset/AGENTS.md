...
| CSS 클래스명 | `.rui-{name}` | `.rui-{name}__{slot}` |
...
2. **CSS 클래스명 패턴이 변경됨**: `defineRecipe`의 `.rui-{name}` → `defineSlotRecipe`의 `.rui-{name}__root`로 변경되므로 React 컴포넌트에서 사용하는 import도 업데이트 필요.
