export const vars = {
  "base": {
    "enabled": {
      "root": {
        "strokeColor": "var(--rui-color-stroke-neutral-muted)",
        "strokeWidth": "1px",
        "cornerRadius": "var(--rui-radius-full)"
      },
      "label": {
        "color": "var(--rui-color-fg-neutral)",
        "fontWeight": "var(--rui-font-weight-medium)"
      },
      "prefixIcon": {
        "color": "var(--rui-color-fg-neutral)"
      },
      "suffixIcon": {
        "color": "var(--rui-color-fg-neutral-subtle)"
      },
      "icon": {
        "color": "var(--rui-color-fg-neutral)"
      },
      "count": {
        "color": "var(--rui-color-fg-neutral-muted)",
        "fontWeight": "var(--rui-font-weight-medium)"
      }
    },
    "pressed": {
      "root": {
        "color": "var(--rui-color-bg-layer-default-pressed)"
      }
    },
    "selected": {
      "root": {
        "strokeWidth": "0px",
        "color": "var(--rui-color-bg-neutral-solid-muted)"
      },
      "label": {
        "color": "var(--rui-color-palette-static-white)",
        "fontWeight": "var(--rui-font-weight-bold)"
      },
      "prefixIcon": {
        "color": "var(--rui-color-palette-static-white)"
      },
      "suffixIcon": {
        "color": "var(--rui-color-palette-static-white)"
      },
      "icon": {
        "color": "var(--rui-color-palette-static-white)"
      },
      "count": {
        "color": "var(--rui-color-palette-static-white-alpha-800)"
      }
    },
    "selectedPressed": {
      "root": {
        "color": "var(--rui-color-bg-neutral-solid-muted-pressed)"
      }
    },
    "disabled": {
      "root": {
        "color": "var(--rui-color-bg-disabled)"
      },
      "label": {
        "color": "var(--rui-color-fg-disabled)"
      },
      "prefixIcon": {
        "color": "var(--rui-color-fg-disabled)"
      },
      "suffixIcon": {
        "color": "var(--rui-color-fg-disabled)"
      },
      "icon": {
        "color": "var(--rui-color-fg-disabled)"
      },
      "count": {
        "color": "var(--rui-color-fg-disabled)"
      }
    }
  },
  "sizeSmall": {
    "enabled": {
      "root": {
        "minHeight": "var(--rui-dimension-x8)",
        "paddingY": "var(--rui-dimension-x1_5)",
        "gap": "var(--rui-dimension-x1)"
      },
      "label": {
        "fontSize": "var(--rui-font-size-t4)",
        "lineHeight": "var(--rui-line-height-t4)"
      },
      "prefixIcon": {
        "size": "var(--rui-dimension-x4)"
      },
      "suffixIcon": {
        "size": "var(--rui-dimension-x3_5)"
      },
      "count": {
        "fontSize": "var(--rui-font-size-t4)"
      }
    }
  },
  "sizeMedium": {
    "enabled": {
      "root": {
        "minHeight": "var(--rui-dimension-x9)",
        "paddingY": "var(--rui-dimension-x2)",
        "gap": "var(--rui-dimension-x1)"
      },
      "label": {
        "fontSize": "var(--rui-font-size-t4)",
        "lineHeight": "var(--rui-line-height-t4)"
      },
      "prefixIcon": {
        "size": "var(--rui-dimension-x4)"
      },
      "suffixIcon": {
        "size": "var(--rui-dimension-x3_5)"
      },
      "count": {
        "fontSize": "var(--rui-font-size-t4)"
      }
    }
  },
  "sizeSmallLayoutWithText": {
    "enabled": {
      "root": {
        "paddingX": "var(--rui-dimension-x3)"
      }
    }
  },
  "sizeSmallLayoutIconOnly": {
    "enabled": {
      "root": {
        "minWidth": "var(--rui-dimension-x8)"
      },
      "icon": {
        "size": "var(--rui-dimension-x4)"
      }
    }
  },
  "sizeMediumLayoutWithText": {
    "enabled": {
      "root": {
        "paddingX": "var(--rui-dimension-x3_5)"
      }
    }
  },
  "sizeMediumLayoutIconOnly": {
    "enabled": {
      "root": {
        "minWidth": "var(--rui-dimension-x9)"
      },
      "icon": {
        "size": "var(--rui-dimension-x4)"
      }
    }
  }
}