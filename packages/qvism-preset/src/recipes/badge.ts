...
const badge = defineSlotRecipe({
  ...
  variants: {
    size: {
      medium: {
        root: {
          // TODO: have some better way to derive `--rui-font-size-limit-min/max` and `108px`
          // NOTE: when updating vars.sizeMedium.enabled.root.maxWidth, update 108px accordingly
          maxWidth: `clamp(calc(108px * var(--rui-font-size-limit-min)), ${vars.sizeMedium.enabled.root.maxWidth}, calc(108px * var(--rui-font-size-limit-max)))`,
      ...
      large: {
        root: {
          // TODO: have some better way to derive `--rui-font-size-limit-min/max` and `120px`
          // NOTE: when updating vars.sizeLarge.enabled.root.maxWidth, update 120px accordingly
          maxWidth: `clamp(calc(120px * var(--rui-font-size-limit-min)), ${vars.sizeLarge.enabled.root.maxWidth}, calc(120px * var(--rui-font-size-limit-max)))`,
