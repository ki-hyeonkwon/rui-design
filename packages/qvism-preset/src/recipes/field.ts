...
export const fieldLabel = defineSlotRecipe({
  ...
  base: {
    ...
    indicatorText: {
      ...
      // TODO: have some better way to derive `--rui-font-size-limit-min/max` and px values
      // NOTE: when updating vars, update px values accordingly
      paddingLeft: `clamp(calc(4px * var(--rui-font-size-limit-min)), ${vars.base.enabled.indicatorText.paddingLeft}, calc(4px * var(--rui-font-size-limit-max)))`,
    ...
    indicatorIcon: {
      ...
      // TODO: have some better way to derive `--rui-font-size-limit-min/max` and px values
      // NOTE: when updating vars, update px values accordingly
      width: `clamp(calc(6px * var(--rui-font-size-limit-min)), ${vars.base.enabled.indicatorIcon.size}, calc(6px * var(--rui-font-size-limit-max)))`,
      height: `clamp(calc(6px * var(--rui-font-size-limit-min)), ${vars.base.enabled.indicatorIcon.size}, calc(6px * var(--rui-font-size-limit-max)))`,
      marginTop: `clamp(calc(4px * var(--rui-font-size-limit-min)), ${vars.base.enabled.indicatorIcon.paddingTop}, calc(4px * var(--rui-font-size-limit-max)))`,
      marginLeft: `clamp(calc(2px * var(--rui-font-size-limit-min)), ${vars.base.enabled.indicatorIcon.paddingLeft}, calc(2px * var(--rui-font-size-limit-max)))`,
