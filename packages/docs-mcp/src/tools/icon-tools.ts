/**
 * Icon Tools for RUI Design MCP Server
 *
 * Provides tools for discovering and searching RUI Design icons.
...
const DOCS_BASE_URL = "https://rui-design.io/docs/foundation/iconography/library";
...
export const listIconsTool: Tool<IconToolContext> = {
  name: "list_icons",
  description:
    "List available RUI Design icons with optional filtering by type, variant, or service",
  async ctx() {
...
export const searchIconsTool: Tool<IconToolContext> = {
  name: "search_icons",
  description: "Search RUI Design icons by keyword. Supports both English and Korean queries.",
  async ctx() {
    const iconData = await loadIconData();
    return { iconData };
  },
