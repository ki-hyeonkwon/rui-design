...
const logger = createTransformLogger("replace-css-rui-typography-variable");
...
/**
 * CSS 변수 값 변환 함수
 * @param value CSS 변수 값 (예: "var(--rui-semantic-typography-label4-regular-font-size)")
 * @returns 변환된 CSS 변수 값 (예: "var(--rui-font-size-t2)")
 */
  ...
  newValue = value.replace(
    ...
    (match, style, weight, property) => {
      try {
        ...
        switch (property) {
          case "font-size":
            newVarName = `--rui-font-size-${newToken.replace(/Regular|Medium|Bold/, "")}`;
            break;
          case "line-height":
            newVarName = `--rui-line-height-${newToken.replace(/Regular|Medium|Bold/, "")}`;
            break;
          case "font-weight":
            if (newToken.includes("Regular")) {
              newVarName = "--rui-font-weight-regular";
            } else if (newToken.includes("Medium")) {
              newVarName = "--rui-font-weight-medium";
            } else if (newToken.includes("Bold")) {
              newVarName = "--rui-font-weight-bold";
            }
            break;
          case "letter-spacing":
...
function processCssFile(source: string, filePath: string): string {
  ...
  try {
    ...
    const postcssPlugin: Plugin = {
      ...
      Declaration(decl) {
        // var(--rui-semantic-typography를 포함하는 선언만 처리
        if (decl.value.includes("var(--rui-semantic-typography")) {
