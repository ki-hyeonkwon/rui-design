...
function handleImports(j: any, root: any, hasTransformedComponents: boolean) {
  ...
  textImports.forEach((path: any) => {
    ...
    if (defaultSpecifier) {
      // 이 import 문을 @rideds/react import로 대체
      path.node.source.value = "@rideds/react";
...
const transform: Transform = (file, api) => {
  const logger = createTransformLogger("replace-custom-rui-text-component");
