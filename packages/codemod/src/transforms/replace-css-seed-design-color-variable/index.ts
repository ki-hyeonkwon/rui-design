import { colorMappings } from "@rideds/migration-index";
import type { Transform } from "jscodeshift";
import postcss, { type Plugin } from "postcss";
import { createTransformLogger } from "../../utils/logger.js";

// transform 최상단에 logger 정의
const logger = createTransformLogger("replace-css-rui-color-variable");
...
function transformCssVarValue(
  ...
  newValue = value.replace(
    /var\(--rui-(semantic|scale|static)-color-([^)]+)\)/g,
    (match, tokenType, tokenName) => {
      try {
        ...
        // CSS 변수 형식으로 변환
        const newVar = `var(--rui-${nextToken.substring(1).replace(/\./g, "-")})`;
        hasChanges = true;
...
function processCssFile(source: string, filePath: string): string {
  ...
  try {
    ...
    const postcssPlugin: Plugin = {
      ...
      Declaration(decl) {
        // var(--rui-semantic-color 또는 var(--rui-scale-color를 포함하는 선언만 처리
        if (
          decl.value.includes("var(--rui-semantic-color") ||
          decl.value.includes("var(--rui-scale-color") ||
          decl.value.includes("var(--rui-static-color")
        ) {
