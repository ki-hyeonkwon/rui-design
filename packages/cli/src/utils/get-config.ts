...
export async function getConfig(cwd: string): Promise<Config> {
  ...
  p.log.error("프로젝트 루트 경로에 `rui.json` 파일이 없어요.");

  const isConfirm = await p.confirm({ message: "rui.json 파일을 생성하시겠어요?" });
  ...
  try {
    ...
    p.log.message("rui.json 파일이 생성됐어요.");
    return configSchema.parse(DEFAULT_INIT_CONFIG);
  } catch (error) {
    throw new CliError({
      message: "rui.json 파일 생성에 실패했어요.",
...
export async function getRawConfig(cwd: string): Promise<Config | null> {
  ...
  try {
    ...
    throw new CliError({
      message: "rui.json 형식이 올바르지 않아요.",
      hint: "https://rui.com/react/getting-started/cli/configuration 문서를 참고해 주세요.",
      cause: error,
    });
  }
}
