...
export const docsCommand = (cli: CAC) => {
  cli
    ...
    .example("rui docs")
    .example("rui docs action-button")
    .example("rui docs react")
    .example("rui docs react/components")
    .example("rui docs react/components/action-button")
          ...
          if (matchedCategory && segments.length >= 2) {
            ...
            if (matchedSection && segments.length >= 3) {
              ...
              if (matchedItem) {
                ...
                if (matched.length === 0) {
                  throw new CliError({
                    ...
                    hint: `\`rui docs ${matchedCategory.id}/${matchedSection.id}\`로 목록을 확인해보세요.`,
                  });
                }
                if (matched.length === 1) {
              ...
              if (matched.length === 0) {
                throw new CliError({
                  message: `${highlight(options.query)}: 문서를 찾을 수 없어요.`,
                  hint: `\`rui docs ${matchedCategory.id}\`로 목록을 확인해보세요.`,
                });
              }
              if (matched.length === 1) {
