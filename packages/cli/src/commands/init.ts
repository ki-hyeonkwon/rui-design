      ...
      try {
        ...
        const relativePath = await (async () => {
          try {
            ...
            stop("rui.json 파일 생성이 중단됐어요.");
            throw error;
          }
        })();

        stop(`rui.json 파일이 ${highlight(relativePath)}에 생성됐어요.`);

        p.log.info(highlight("rui add {component} 명령어로 컴포넌트를 추가해보세요!"));
        p.log.info(
          highlight("rui add 명령어로 추가할 수 있는 모든 컴포넌트를 확인해보세요."),
        );

        p.note(
          dedent(`RUI Design CLI는 개선을 위해 익명 사용 데이터를 수집해요.

      비활성화하려면:
        • rui.json에서 ${highlight('"telemetry": false')}로 설정
        • ${highlight("DISABLE_TELEMETRY=true")} 환경 변수 설정

      자세한 내용: https://rui.com/react/getting-started/cli/configuration#telemetry`),
          "Telemetry 안내",
        );
        ...
        handleCliError(error, {
          defaultMessage: "rui.json 파일 생성에 실패했어요.",
