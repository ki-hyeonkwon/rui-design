...
export const compatCommand = (cli: CAC) => {
  cli
    ...
    .example("rui compat")
    .example("rui compat -c action-button")
    .example("rui compat ui:action-button ui:alert-dialog")
    .example("rui compat --all")
      ...
      try {
        ...
        const resolvedTargetItemKeys = Array.isArray(targetItemKeys)
          ...
          : await (async () => {
              ...
              if (!rawConfig) {
                throw new CliError({
                  message: "rui.json 파일이 없어 설치된 스니펫 경로를 알 수 없어요.",
                  hint: "`rui init`으로 설정을 만든 뒤 실행하거나, `--all`/`-c`로 검사 대상을 직접 지정해주세요.",
                });
              }
