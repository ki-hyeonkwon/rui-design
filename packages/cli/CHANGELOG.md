...
- d25a0d6: LLM 친화적 문서 링크를 위한 `docs` 명령어 추가
  ...
  - 컴포넌트 및 파운데이션에 대한 문서 URL, llms.txt URL, GitHub raw 스니펫 URL을 출력하는 `rui docs [query]` 명령어를 추가합니다.
...
- 50cd41e: - `rui compat` 명령어를 추가해 현재 프로젝트의 스니펫과 `@rideds/react`, `@rideds/css` 버전 호환성을 비대화형으로 점검할 수 있도록 개선합니다.
...
- 1af88d1: - CLI 실패 원인 표시를 개선하고 `--verbose` 상세 진단 출력을 추가합니다.
  - `rui.json`이 없을 때 외부 명령 실행 대신 내부 초기화 로직으로 설정 파일을 생성합니다.
...
- 950c9e1: **`add` 명령어 사용 방식을 변경합니다.**
  ...
  ```sh
  rui add ui:action-button breeze:animate-number # ui 이외 레지스트리의 항목도 추가할 수 있게 되었습니다.
  ```

  - 특정 레지스트리에 있는 모든 항목 추가

  ```sh
  rui add-all ui lib breeze
  ```
...
- 9b271d6: snippet의 deprecated 여부를 기록합니다. cli add 명령 실행 시 snippet 목록에서 deprecate 여부를 표시하고, `--all`로 모든 스니펫 추가 시 기본적으로 deprecated snippet을 제외합니다.
  ...
  ```sh
  rui add --all # deprecated snippet 제외
  ```

  ```sh
  rui add --all --include-deprecated # deprecated snippet 포함
  ```
...
- default resolve path를 `rui/ui`로 변경해요.
