...
컴포넌트는 그 이후에 화면별로 적용하거나, 컴포넌트별로 적용하는 것과 같이 점진적 적용이 가능합니다.
...
```package-install
@rideds/react @rideds/css
```

- `@rideds/react`는 리액트 컴포넌트 라이브러리가 전부 포함되어 있습니다.
  - 기존 `sprout` 패키지들을 대체합니다.
  - 컴포넌트 마이그레이션 전까지는 기존 패키지들을 사용해주세요.
- `@rideds/css`는 스타일 관련된 모든 것이 포함되어 있습니다. (vars, theming, recipe, css, etc.)
  - 기존 `@rideds/stylesheet` 라이브러리를 대체합니다.
  - 기존 `@rideds/design-token` 라이브러리를 대체합니다.
...
<Callout>
V2 `@rideds/stylesheet` 패키지의 `global.css`와 상호운용이 가능합니다.
</Callout>

<Callout type="warn">
번들러 플러그인([webpack](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/webpack-plugin), [vite](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/vite-plugin), [rsbuild](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/rsbuild-plugin))을 사용하는 경우에는 `all.css` 대신 `base.css`를 사용해주세요.
</Callout>

```ts title="index.tsx"
import "@rideds/css/all.css"; // [!code ++]
```
...
```html
<!-- [!code --] -->
<html data-seed data-rui-scale-color="light" />
<!-- [!code ++] -->
<html data-seed data-rui-color-mode="system" data-rui-user-color-scheme="light" />
```

- `data-seed`: Seed Design 전역 테마 적용 (V3에서 그대로 유지)
- `data-rui-color-mode`: Seed Design 테마 강제 적용
  - `light-only`, `dark-only`, `system` 중 하나를 선택할 수 있습니다.
- `data-rui-user-color-scheme`: 유저 기기의 설정
  - `light`, `dark` 중 하나의 값을 가집니다.
  - v2에서 `data-rui-scale-color` 속성을 사용하던 코드가 있다면 `data-rui-user-color-scheme` 속성으로 변경해주세요.
...
- [@rideds/vite-plugin](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/vite-plugin)
...
- [@rideds/rsbuild-plugin](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/rsbuild-plugin)
...
- [@rideds/webpack-plugin](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/webpack-plugin)
...
`@rideds/css/theming` 패키지에서 제공하는 `generateThemingScript` 함수를 사용해서 스크립트를 생성해요.

```ts title="index.tsx"
import { generateThemingScript } from "@rideds/css/theming";
```
...
우선 현재 프로젝트에서 `@rideds/design-token` 패키지를 사용하는 용례를 분석해야합니다.

- `@rideds/design-token` 패키지의 vars를 사용하는 용례
- `@rideds/design-token` 패키지의 vars를 확장해서 사용하는 컴포넌트
- 위 컴포넌트를 사용중인 코드 용례
- `@rideds/stylesheet` 패키지를 사용하는 용례
- `--rui-` prefix로 시작하는 CSS Variable을 사용하는 용례
- 사용중인 CSS 라이브러리와 `@rideds/design-token` 패키지와 결합해서 사용하는 용례
...
1. `@rideds/design-token` 패키지의 vars 변수를 사용하는 용례
2. `@rideds/design-token` 패키지의 vars 변수를 확장해서 사용하는 컴포넌트
3. 2번 컴포넌트를 사용중인 코드 용례 
4. `@rideds/styleesheet` 패키지를 사용하는 용례
5. `--rui-` prefix로 시작하는 CSS Variable을 사용하는 용례
6. 사용중인 CSS 라이브러리와 `@rideds/design-token` 패키지와 결합해서 사용하는 용례
...
- [replace-custom-rui-typography](https://rui-design.io/react/developer-tools/codemods/available-transforms#replace-custom-rui-typography) 커스텀 transform을 사용해서 마이그레이션 할 수 있습니다.

```ts title="typography.ts"
import type { TextVariant } from '@rideds/css/recipes/text';

import { vars as typographyVars } from '@rideds/css/vars/component/typography';

/**
 * @see https://rui-design.io/docs/foundation/typography/overview
 * @example
 * typography.t1Bold
...
```package-install
npx @rideds/codemod@latest <transform> <target_path> <옵션>
```
...
- [@rideds/tailwind3-plugin](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/tailwind3-plugin)
- [@rideds/tailwind4-theme](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/tailwind4-theme)
...
- Emotion, Vanilla Extract CSS와 같은 CSS-in-JS 라이브러리는 `@rideds/design-token`을 사용하는 경우가 대부분입니다.
- [replace-rui-token-vars](/react/developer-tools/codemods/available-transforms#replace-rui-token-vars) transform을 사용해서 일괄적으로 마이그레이션 할 수 있습니다.
- 해당 transform은 `color`와 `typography` 두 개 동시에 변경합니다.

#### Native CSS

- 기본적으로 `--rui-` prefix가 붙은 CSS Variable을 사용하는 경우가 있습니다.
- [replace-css-rui-color-variable](/react/developer-tools/codemods/available-transforms#replace-css-rui-color-variable) CSS transform도 제공합니다.
...
이론상 이때부터는 `@rideds/design-token` 패키지와 `@rideds/stylesheet` 패키지를 제거할 수 있습니다.
또한 V3 컴포넌트를 점진적 사용이 가능합니다.

### V3 Compat Script 제거

```html
<script src="https://assets.krrt.io/rui-design/rui-design-critical-script/latest/lib/index.js"></script>
```
...
- `@vanilla-extract/vite-plugin`와 `@rideds/vite-plugin`을 같이 사용하는 경우 빌드가 실패할 수 있습니다.
