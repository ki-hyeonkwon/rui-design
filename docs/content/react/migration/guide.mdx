...
- `@rui/react`는 리액트 컴포넌트 라이브러리가 전부 포함되어 있습니다.
  - 기존 `sprout` 패키지들을 대체합니다.
  - 컴포넌트 마이그레이션 전까지는 기존 패키지들을 사용해주세요.
- `@rui/css`는 스타일 관련된 모든 것이 포함되어 있습니다. (vars, theming, recipe, css, etc.)
  - 기존 `@rui/stylesheet` 라이브러리를 대체합니다.
  - 기존 `@rui/design-token` 라이브러리를 대체합니다.
...
<Callout>
V2 `@rui/stylesheet` 패키지의 `global.css`와 상호운용이 가능합니다.
</Callout>

<Callout type="warn">
번들러 플러그인([webpack](https://github.com/rui-design/rui-design/tree/dev/packages/webpack-plugin), [vite](https://github.com/rui-design/rui-design/tree/dev/packages/vite-plugin), [rsbuild](https://github.com/rui-design/rui-design/tree/dev/packages/rsbuild-plugin))을 사용하는 경우에는 `all.css` 대신 `base.css`를 사용해주세요.
</Callout>

```ts title="index.tsx"
import "@rui/css/all.css"; // [!code ++]
```
...
- [@rui/vite-plugin](https://github.com/rui-design/rui-design/tree/dev/packages/vite-plugin)
...
- [@rui/rsbuild-plugin](https://github.com/rui-design/rui-design/tree/dev/packages/rsbuild-plugin)
...
- [@rui/webpack-plugin](https://github.com/rui-design/rui-design/tree/dev/packages/webpack-plugin)
...
`@rui/css/theming` 패키지에서 제공하는 `generateThemingScript` 함수를 사용해서 스크립트를 생성해요.

```ts title="index.tsx"
import { generateThemingScript } from "@rui/css/theming";
```
...
우선 현재 프로젝트에서 `@rui/design-token` 패키지를 사용하는 용례를 분석해야합니다.

- `@rui/design-token` 패키지의 vars를 사용하는 용례
- `@rui/design-token` 패키지의 vars를 확장해서 사용하는 컴포넌트
- 위 컴포넌트를 사용중인 코드 용례
- `@rui/stylesheet` 패키지를 사용하는 용례
- `--rui-` prefix로 시작하는 CSS Variable을 사용하는 용례
- 사용중인 CSS 라이브러리와 `@rui/design-token` 패키지와 결합해서 사용하는 용례
...
1. `@rui/design-token` 패키지의 vars 변수를 사용하는 용례
2. `@rui/design-token` 패키지의 vars 변수를 확장해서 사용하는 컴포넌트
3. 2번 컴포넌트를 사용중인 코드 용례 
4. `@rui/styleesheet` 패키지를 사용하는 용례
5. `--rui-` prefix로 시작하는 CSS Variable을 사용하는 용례
6. 사용중인 CSS 라이브러리와 `@rui/design-token` 패키지와 결합해서 사용하는 용례
...
import type { TextVariant } from '@rui/css/recipes/text';

import { vars as typographyVars } from '@rui/css/vars/component/typography';
...
```package-install
npx @rui/codemod@latest <transform> <target_path> <옵션>
```
...
- [@rui/tailwind3-plugin](https://github.com/rui-design/rui-design/tree/dev/packages/tailwind3-plugin)
- [@rui/tailwind4-theme](https://github.com/rui-design/rui-design/tree/dev/packages/tailwind4-theme)
...
- Emotion, Vanilla Extract CSS와 같은 CSS-in-JS 라이브러리는 `@rui/design-token`을 사용하는 경우가 대부분입니다.
...
이론상 이때부터는 `@rui/design-token` 패키지와 `@rui/stylesheet` 패키지를 제거할 수 있습니다.
...
- `@vanilla-extract/vite-plugin`와 `@rui/vite-plugin`을 같이 사용하는 경우 빌드가 실패할 수 있습니다.
