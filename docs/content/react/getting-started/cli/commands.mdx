---
title: Commands
description: "@rideds/cli 명령어를 안내해요."
---

## 공통 동작

- `add`, `add-all`은 `rui.json` 설정 파일을 사용해요.
- 설정 파일이 없거나 읽을 수 없으면 생성할지 먼저 물어봐요.
- 항목 추가 시 필요한 npm 패키지 의존성은 자동으로 설치돼요.
- `add`, `add-all` 실행 시 스니펫이 요구하는 `@rideds/react`, `@rideds/css` 버전과 현재 프로젝트 버전이 다르면 경고를 보여줘요.
...
```package-install
npx @rideds/cli@latest add [...item-ids]
```
...
기본값은 배포 환경 기준 [`https://rui-design.io`](https://rui-design.io)입니다.

이 옵션은 특정 SEED React 패키지와 호환되는 스니펫이 필요한 경우 활용할 수 있습니다.

```package-install
npx @rideds/cli@latest add --baseUrl https://1-0.rui.pages.dev
```

```package-install
npx @rideds/cli@latest add --baseUrl https://1-1.rui.pages.dev
```
...
```package-install
npx @rideds/cli@latest add-all [...registry-ids]
```

모든 레지스트리의 모든 항목을 추가하려면 `--all` 옵션을 사용하세요.

```package-install
npx @rideds/cli@latest add-all --all # 모든 레지스트리의 모든 항목 추가
```

특정 레지스트리의 모든 항목을 추가할 수도 있습니다.

```package-install
npx @rideds/cli@latest add-all breeze # breeze 레지스트리의 모든 항목 추가
```

deprecated 항목을 포함하려면 `--include-deprecated` 옵션을 사용하세요.

```package-install
npx @rideds/cli@latest add-all ui --include-deprecated # ui 레지스트리의 모든 항목 추가 (deprecated 포함)
```
...
현재 프로젝트의 `@rideds/react`, `@rideds/css` 버전과 스니펫 요구 버전이 호환되는지 검사해요.

```package-install
npx @rideds/cli@latest compat
```

기본값은 `rui.json`의 `path` 아래에 이미 추가된 스니펫만 검사해요.
