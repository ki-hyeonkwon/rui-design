...
- `add`, `add-all` 실행 시 스니펫이 요구하는 `@rui/react`, `@rui/css` 버전과 현재 프로젝트 버전이 다르면 경고를 보여줘요.
...
```package-install
npx @rui/cli@latest add [...item-ids]
```
...
```package-install
npx @rui/cli@latest add --baseUrl https://1-0.rui.pages.dev
```

```package-install
npx @rui/cli@latest add --baseUrl https://1-1.rui.pages.dev
```
...
```package-install
npx @rui/cli@latest add-all [...registry-ids]
```

모든 레지스트리의 모든 항목을 추가하려면 `--all` 옵션을 사용하세요.

```package-install
npx @rui/cli@latest add-all --all # 모든 레지스트리의 모든 항목 추가
```

특정 레지스트리의 모든 항목을 추가할 수도 있습니다.

```package-install
npx @rui/cli@latest add-all breeze # breeze 레지스트리의 모든 항목 추가
```

deprecated 항목을 포함하려면 `--include-deprecated` 옵션을 사용하세요.

```package-install
npx @rui/cli@latest add-all ui --include-deprecated # ui 레지스트리의 모든 항목 추가 (deprecated 포함)
```
...
현재 프로젝트의 `@rui/react`, `@rui/css` 버전과 스니펫 요구 버전이 호환되는지 검사해요.
