---
title: Configuration
description: rui.json 설정을 안내해요.
---


## rui.json 만들기

`rui.json` 파일은 CLI로 생성하거나 직접 추가할 수 있어요.
...
<Callout>
`rui.json`은 엄격하게 파싱돼요. 문서에 없는 키를 임의로 추가하면 CLI 실행 시 오류가 날 수 있어요.
</Callout>
...
| 옵션 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `path` | `string` | 없음 (init에서는 `./rui`) | 스니펫이 생성될 루트 경로 |
...
<Files>
  <Folder name="rui (path)" defaultOpen>
...
RUI Design CLI는 개선을 위해 익명 사용 데이터를 수집해요.
