---
title: Theming
description: RUI Design의 테마 시스템을 이해하고 활용하는 방법을 알아봅니다.
---

## 개요

RUI Design은 라이트 모드와 다크 모드를 지원하는 테마 시스템을 제공합니다.
테마는 HTML 요소의 `data-rui-color-mode`와 `data-rui-user-color-scheme` 속성을 통해 제어됩니다.

두 속성의 역할과 차이점을 이해하고, 전역 테마 설정 방법과 특정 영역에만 테마를 오버라이딩하는 방법을 알아봅니다.

## 테마 속성 이해하기

RUI Design의 테마 시스템은 두 가지 `data-*` 속성을 사용합니다.
...
```html
<!-- 시스템 설정 따르기 -->
<html data-rui-color-mode="system" />

<!-- 라이트 모드로 고정 -->
<html data-rui-color-mode="light-only" />

<!-- 다크 모드로 고정 -->
<html data-rui-color-mode="dark-only" />
```
...
이 값은 `data-rui-color-mode="system"`일 때만 의미를 가지며, `prefers-color-scheme` 미디어 쿼리를 통해 자동으로 업데이트됩니다.

```html
<!-- 시스템 설정을 따르도록 설정했으며, 현재 사용자는 라이트 모드를 사용 중 -->
<html data-rui-color-mode="system" data-rui-user-color-scheme="light"></html>
```
...
/* 다크 모드가 적용되는 경우 */
:root[data-rui-color-mode="system"][data-rui-user-color-scheme="dark"],
...
특정 영역이나 컴포넌트에만 다른 테마를 적용하고 싶을 때는 해당 요소에 `data-rui-color-mode`에 `light-only`, `dark-only` 또는 `system` 값을 설정합니다.
...
이런 경우 `data-rui-color-mode`와 `data-rui-user-color-scheme` 속성을 감지하여 사용할 수 있습니다.
...
- `MutationObserver`를 사용하여 `data-rui-color-mode`와 `data-rui-user-color-scheme` 속성의 변경을 감지합니다.
