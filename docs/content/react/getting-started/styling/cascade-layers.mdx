...
.rui-floating-action-button__root {
  background: var(--rui-color-bg-brand-solid);
}
...
```css title="style.css"
@import "@rideds/css/base.layered.css";
```
...
번들러의 module resolution 설정에 `rui-layered` condition을 추가하세요. 이 설정을 통해 SEED React 컴포넌트가 내부적으로 CSS를 import할 때, 자동으로 layered 버전을 사용합니다.

<Tabs groupId="bundler" persist items={['Vite', 'Rsbuild', 'Webpack']}>
...
export default defineConfig({
  resolve: {
    conditions: ["rui-layered"], // [!code highlight]
  },
  // ...
});
```
...
export default defineConfig({
  resolve: {
    conditionNames: ["rui-layered", "..."], // [!code highlight]
  },
  // ...
});
```
