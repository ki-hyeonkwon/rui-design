...
@media (hover: hover) {
  .rui-action-button--variant_brandSolid:is(:hover, [data-hover]) {
    background: var(--rui-color-bg-brand-solid-pressed);
  }
}

/* 터치 환경: active 시 pressed 색상 적용 */
@media (hover: none) {
  .rui-action-button--variant_brandSolid:is(:active, [data-active]) {
    background: var(--rui-color-bg-brand-solid-pressed);
  }
}
```
