...
```package-install
npx @rui/cli@latest add ui:list
```
...
@rui/react 패키지에서 제공하는 `List`와 `ListHeader` 컴포넌트는 다음과 같은 구조로 사용됩니다.
...
```tsx
import { ListHeader, List, Icon } from "@rui/react";
...
```tsx
import { List as SeedList } from "@rui/react";
