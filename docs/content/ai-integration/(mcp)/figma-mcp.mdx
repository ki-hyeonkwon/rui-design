...
```bash
codex mcp add rui-design-figma \
  --env FIGMA_PERSONAL_ACCESS_TOKEN=${FIGMA_PERSONAL_ACCESS_TOKEN} \
  -- bunx -y @rideds/mcp@latest
```
...
| `retrieve_color_variable_names` | RUI Design 색상 변수 이름 목록을 반환합니다. |
...
```bash
bunx --bun @rideds/mcp@latest --config /path/to/config.ts
```

### 지원하는 설정 파일 형식

`@rideds/mcp`는 다음 확장자의 설정 파일을 지원합니다:
...
SEED Figma Slider 컴포넌트를 SEED React `<Slider />`로 매핑하는 [ComponentHandler 예시](https://github.com/ki-hyeonkwon/rui-design/blob/dev/packages/figma/src/codegen/targets/react/component/handlers/slider.ts)를 확인할 수 있습니다.
...
```typescript
import { type react } from "@rideds/figma";
