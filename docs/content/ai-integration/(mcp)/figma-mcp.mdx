...
```bash
bunx --bun @rui/mcp@latest --config /path/to/config.ts
```

### 지원하는 설정 파일 형식

`@rui/mcp`는 다음 확장자의 설정 파일을 지원합니다:
...
SEED Figma Slider 컴포넌트를 SEED React `<Slider />`로 매핑하는 [ComponentHandler 예시](https://github.com/rui-design/rui-design/blob/dev/packages/figma/src/codegen/targets/react/component/handlers/slider.ts)를 확인할 수 있습니다.
...
```typescript
import { type react } from "@rui/figma";
