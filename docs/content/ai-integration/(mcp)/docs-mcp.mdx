...
[`@rui/docs-mcp`](https://github.com/rui-design/rui-design/tree/dev/packages/docs-mcp)는 RUI Design 문서를 위한 공식 MCP(Model Context Protocol) 서버입니다.
...
```sh
claude mcp add rui-docs -- npx -y @rui/docs-mcp
```
...
```sh
codex mcp add rui-docs -- npx -y @rui/docs-mcp
```
...
`@rui/docs-mcp`는 npm 패키지로 설치하여 커스텀 MCP 서버에 통합할 수 있습니다.
