---
title: Docs MCP
description: AI Agent가 RUI Design 문서에 직접 접근할 수 있는 MCP 서버를 제공합니다.
---

## 소개

[`@rideds/docs-mcp`](https://github.com/ki-hyeonkwon/rui-design/tree/dev/packages/docs-mcp)는 RUI Design 문서를 위한 공식 MCP(Model Context Protocol) 서버입니다.
...
Figma 연결 없이 독립적으로 실행되며, RUI Design을 사용하는 프로젝트에서 AI 도구의 문서 이해도를 높일 수 있습니다.
...
```sh
claude mcp add rui-docs -- npx -y @rideds/docs-mcp
```
...
```sh
codex mcp add rui-docs -- npx -y @rideds/docs-mcp
```
...
응답:
```
# RUI Design React - Getting Started Topics
...
RUI Design의 Rootage 스펙(디자인 토큰 및 컴포넌트 스펙의 원본 JSON 데이터)에 접근합니다.
...
RUI Design 아이콘을 검색하고 사용법을 확인합니다.
...
View in browser: https://rui-design.io/docs/foundation/iconography/library?search=화살표
...
View this icon: https://rui-design.io/docs/foundation/iconography/library?icon=icon_arrow_left_line
```
...
`@rideds/docs-mcp`는 npm 패키지로 설치하여 커스텀 MCP 서버에 통합할 수 있습니다.
