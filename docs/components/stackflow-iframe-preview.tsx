...
function getStackflowSpaUrl(path: string): string {
  const baseURL = (() => {
    ...
    // Branch previews only — deployment-hash based preview URLs (e.g. 340c0934.rui.pages.dev)
    // cannot be mapped because Cloudflare generates independent deployment hashes per project.
    // Docs URL: https://<branch>.rui.pages.dev
    // QA URL:   https://<branch>.rui-design-stackflow-spa.pages.dev
    if (window.location.hostname.endsWith(".rui.pages.dev")) {
      const branch = window.location.hostname.split(".")[0];

      return `https://${branch}.rui-design-stackflow-spa.pages.dev`;
    }

    // Production
    return "https://qa.rui-design.io";
  })();
